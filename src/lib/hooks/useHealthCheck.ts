...
export const useHealthCheck = (): HealthCheckState & {
  ...
  const handleDataEvent = useCallback(
    (data: SensorData) => {
      ...
      // Ensure the state is updated only when sober or drunk is 0
      if (data.sober === 1 && data.drunk === 1) {
        console.log("⚠️ Ignoring intermediate alcohol state:", data);
        return;
      }
      ...
      console.log("🚀 Finalized alcohol data update:", alcoholStatus);
    },
    [
      state.currentState,
      state.stabilityTime,
